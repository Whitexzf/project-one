			...
			public void keyPressed(KeyEvent e){
				if((e.getKeyChar()==KeyEvent.VK_ENTER )){
					...
					if(epwd.isEmpty()&&!(pwd.equals(epwd))){
						...
						boolean b = ud.isexit(userText.getText());
						if(b){
							...
							ub=new UserBean();
							ub.setUser_name(users);
							ub.setUser_pwd(pwd);
							UsersDAOinf uda=new UsersDAOImpl();
							if(uda.update(ub)){
								JOptionPane.showMessageDialog(logo_Frame, "用户名添加成功！");
							}else{
								JOptionPane.showMessageDialog(logo_Frame, "用户名添加失败！")
								;
							}
						}
					}else{
						JOptionPane.showMessageDialog(logo_Frame, "必须输入用户名！","输入用户名",JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		});
